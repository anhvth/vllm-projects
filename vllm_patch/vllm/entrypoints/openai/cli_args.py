...
class FrontendArgs(BaseFrontendArgs):
    ...
    managed_weight_sync: bool = False
    """Enable unsafe managed weight-sync control endpoints on the OpenAI
    compatible server. Requires VLLM_SERVER_DEV_MODE=1 by default."""
    managed_weight_sync_prefix: str = "/managed"
    """URL prefix for managed weight-sync control endpoints."""
    managed_weight_sync_require_dev_mode: bool = True
    """Require VLLM_SERVER_DEV_MODE=1 when managed weight-sync endpoints are
    enabled."""
...
def validate_parsed_serve_args(args: argparse.Namespace):
    ...
    if args.managed_weight_sync:
        if args.managed_weight_sync_require_dev_mode and not envs.VLLM_SERVER_DEV_MODE:
            raise TypeError(
                "Error: --managed-weight-sync requires VLLM_SERVER_DEV_MODE=1. "
                "Managed weight-sync endpoints are unsafe and must not be "
                "exposed to untrusted networks."
            )
        prefix = args.managed_weight_sync_prefix
        if not prefix.startswith("/"):
            raise ValueError("Error: --managed-weight-sync-prefix must start with '/'.")
